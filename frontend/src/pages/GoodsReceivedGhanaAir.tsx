...
export default function GoodsReceivedGhanaAir() {
  ...
  const fetchData = async (currentPage = 1, isNewSearch = false) => {
    ...
    try {
      ...
      const response = await warehouseService.getGhanaAirGoods(params);
      ...
      console.error("Failed to fetch Ghana air goods:", error);
  ...
  const fetchStats = async () => {
    try {
      const response = await warehouseService.getGhanaAirStatistics();
      setStats(response.data);
    } catch (error) {
      console.error("Failed to fetch Ghana air statistics:", error);
    }
  };
      ...
      <div className="flex flex-col space-y-4 lg:flex-row lg:items-center lg:justify-between lg:space-y-0">
        <div>
          <h1 className="text-xl lg:text-2xl font-semibold text-foreground">
            Ghana Warehouse - Air Cargo
          </h1>
          <p className="text-muted-foreground text-sm lg:text-base">
            Manage air cargo goods received in Ghana warehouse
          </p>
        </div>

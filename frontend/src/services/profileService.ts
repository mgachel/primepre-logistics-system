...
export const profileService = {
  ...
  async updateProfile(profileData: ProfileUpdateData): Promise<ApiResponse<ProfileData>> {
    const response = await apiClient.put<ProfileData>('/api/auth/profile/', profileData);
    return {
      data: response.data,
      success: true,
    };
  ...
  async changePassword(passwordData: PasswordChangeData): Promise<ApiResponse<{ message: string }>> {
    const response = await apiClient.post<{ message: string }>('/api/auth/password-change/', passwordData);
    return {
      data: response.data,
      success: true,
    };
  ...
  async validatePassword(password: string): Promise<boolean> {
    try {
      ...
      const apiError = error as { response?: { data?: { old_password?: string[] } } };

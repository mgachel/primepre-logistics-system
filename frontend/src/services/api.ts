  ...
  private async request<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Try to build a helpful message from DRF-style field errors
        let message = "";
        if (errorData && typeof errorData === "object") {
          const asObj = errorData as Record<string, unknown>;
          if (typeof asObj.detail === "string") {
            message = asObj.detail as string;
          } else {
            const parts: string[] = [];
        ...
        if (!message) message = `HTTP error! status: ${response.status}`;
